...
def calculate_tax(doc_name=None, totalincome=None):
    emp_id, deduction = frappe.db.get_value(
        ...
        doc_name,
        ["employee", "total_declared_amount"],
    )
    total_income = None
    if totalincome:
        total_income = float(totalincome)
    else:
        total_income_list = frappe.db.get_list(
    ...
    template_dict = {
        ...
        "old_cess": old_cess,
        "new_cess": new_cess,
        "old_total_tax": old_regime_tax + old_cess,
        "new_total_tax": new_regime_tax + new_cess,
    }
    template = frappe.render_template(
        "cn_indian_payroll/cn_indian_payroll/tax_calculator.html",
        {"dict": template_dict},
    )
...
def calculate_hra_exemption(emp_id, monthly_house_rent):
    ...
    sm_doc = frappe.get_doc("Employee Salary Master", sm_doc_name[0][0])
    sm_data = {}
