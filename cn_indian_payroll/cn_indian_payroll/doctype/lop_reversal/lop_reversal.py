...
class LOPReversal(Document):
	...
	def on_submit(self):
		...
		self.reimbursement_accrual_update()
		self.bonus_accrual_update()
	...
	def reimbursement_accrual_update(self):
		lop_reversal = frappe.get_all(
			...
			fields=["name", "salary_component", "amount", "payment_days"],
		)

		for record in lop_reversal:
			each_doc = frappe.get_doc("Employee Benefit Accrual", record.name)
			component = frappe.get_doc("Salary Component", each_doc.salary_component)

			# Only adjust if depends_on_payment_days
			if component.depends_on_payment_days:


				lop_reversal_amount = (
					round(each_doc.amount) / each_doc.payment_days
				) * self.number_of_days
	...
	def bonus_accrual_update(self):
		...
		for bonus in lop_reversal_bonus:
			...
			component=frappe.get_doc("Salary Component",each_doc_bonus.salary_component)
			if component.depends_on_payment_days:
