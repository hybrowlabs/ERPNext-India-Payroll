...
class SalaryAdjustments(Document):
    ...
    def insert_additional_salary(self):
        if not (self.earning_arrear_breakup or self.deduction_arrear_breakup):
            return

        # Create Additional Salary for earnings
        for row in self.earning_arrear_breakup:
            additional_salary = frappe.new_doc("Additional Salary")
            additional_salary.employee = self.employee
            additional_salary.salary_component = row.salary_component
            additional_salary.amount = row.amount
            additional_salary.company = self.company
            additional_salary.payroll_date = self.additional_salary_date
            additional_salary.currency = "INR"
            additional_salary.ref_doctype = "Salary Adjustments"
            additional_salary.ref_docname = self.name
            additional_salary.insert()
            additional_salary.submit()

        # Create Additional Salary for deductions
        for row in self.deduction_arrear_breakup:
            additional_salary = frappe.new_doc("Additional Salary")
            additional_salary.employee = self.employee
            additional_salary.salary_component = row.salary_component
            additional_salary.amount = row.amount
    ...
    def insert_breakup_table(self):
        ...
        earning_arrear_breakup = []
        deduction_arrear_breakup = []
        ...
        for new_deduction in new_salary_slip.deductions:
            ...
            if component_doc.custom_is_arrear == 1:
                deduction_arrear_breakup.append(
                    {
                        "salary_component": component_doc.name,
                        "amount": round(
        ...
        self.set("earning_arrear_breakup", [])
        self.set("deduction_arrear_breakup", [])

        for row in earning_arrear_breakup:
            self.append("earning_arrear_breakup", row)

        for row in deduction_arrear_breakup:
            self.append("deduction_arrear_breakup", row)
