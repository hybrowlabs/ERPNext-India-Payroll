...
def get_columns(filters):
    ...
    for component in salary_components:
        ...
        if (
            ...
            and (
                get_doc.custom_regime == "All" or get_doc.custom_regime == "New Regime"
            )
        ):
            columns.append(
                {
...
def get_salary_slip_data(filters=None):
    ...
    salary_slips = frappe.get_all(
        "Salary Slip", filters=conditions, fields=["*"], order_by="posting_date asc"
    )
    ...
    for each_doc in declarations:
        ...
        if each_doc.custom_tax_regime == "Old Regime":
            ...
            get_doc = frappe.get_doc(
                "Employee Tax Exemption Declaration", each_doc.name
            )
            ...
            preventive_health_checkup_self[each_doc.employee] = form_data.get(
                "mpAmount6", 0
            )

            medical_treatment_insurance[each_doc.employee] = form_data.get(
                "fourValue", 0
            )
            medical_treatment_disease[each_doc.employee] = form_data.get(
                "fiveNumber", 0
            )
            ...
            interest_saving_account[each_doc.employee] = form_data.get(
                "seventeenNumber", 0
            )
    ...
    for slip in salary_slips:
        ...
        row = {
            ...
            if slip.employee_name != previous_employee_name
            else "",
            "doj": get_employee.date_of_joining
            if get_employee.date_of_joining != previous_doj
            else "",
            "email": get_employee.company_email
            if get_employee.company_email != previous_company_email
            else "",
            "department": get_employee.department
            if get_employee.department != previous_department
            else "",
            "designation": get_employee.designation
            if get_employee.designation != previous_designation
            else "",
            "pan": get_employee.pan_number
            if get_employee.pan_number != previous_pan
            else "",
        ...
        for earning in earnings:
            ...
            if (
                ...
                employee_totals.setdefault(slip.employee, {}).setdefault(
                    component_key, 0
                )
        ...
        (
            ...
            previous_company_email,
        ) = (
            slip.employee,
            get_employee.date_of_joining,
            get_employee.employee_name,
            get_employee.company_email,
        )
        previous_department, previous_designation, previous_pan = (
            get_employee.department,
            get_employee.designation,
            get_employee.pan_number,
        )

        next_slip_index = salary_slips.index(slip) + 1
        if (
            next_slip_index >= len(salary_slips)
            or salary_slips[next_slip_index].employee != slip.employee
        ):
            actual_row = {
                "employee": "Actual",
                "salary_slip_id": "",
            ...
            projection_row = get_projection(
                slip.employee,
                employee_totals.get(slip.employee, {}),
                employee_counts[slip.employee],
                filters.get("payroll_period"),
            )
            ...
            combined_row = {
                ...
                "education_allowance_exemption": education_allowance_map.get(
                    slip.employee, 0
                ),
                "hostel_allowance_exemption": hostel_allowance_map.get(
                    slip.employee, 0
                ),
                "uniform_allowance_exemption": uniform_allowance_map.get(
                    slip.employee, 0
                ),
                ...
                "mediclaim_parents_below": mediclaim_parents_below.get(
                    slip.employee, 0
                ),
                "mediclaim_parents_senior": mediclaim_parents_senior.get(
                    slip.employee, 0
                ),
                "preventive_health_checkup": preventive_health_checkup.get(
                    slip.employee, 0
                ),
                "preventive_health_checkup_self": preventive_health_checkup_self.get(
                    slip.employee, 0
                ),
                "medical_treatment_insurance": medical_treatment_insurance.get(
                    slip.employee, 0
                ),
                "medical_treatment_disease": medical_treatment_disease.get(
                    slip.employee, 0
                ),
                ...
                "interest_saving_account": interest_saving_account.get(
                    slip.employee, 0
                ),
            ...
            for key in employee_totals.get(slip.employee, {}):
                combined_row[key] = employee_totals[slip.employee].get(
                    key, 0
                ) + projection_row.get(key, 0)
                combined_row["total_income"] += combined_row[key]

            combined_row["total_income"] += (
                combined_row["loan_perquisite"]
                + standard_deduction_old
                + standard_deduction_new
            )

            combined_row["annual_taxable_income"] = max(
                combined_row["total_income"] - total_declaration.get(slip.employee, 0),
                0,
            )
