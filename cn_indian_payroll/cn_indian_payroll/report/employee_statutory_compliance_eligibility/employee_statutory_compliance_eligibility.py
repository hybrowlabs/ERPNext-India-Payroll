...
def execute(filters=None):
	columns = get_columns()
	data = get_all_employee(filters)
	chart = get_pf_pt_chart(data)
	message = "PF vs PT Applicability Summary"
...
def get_pf_pt_chart(data):
	"""Generate PF vs PT applicability graph"""
	pf_yes = sum(1 for d in data if d.get("pf_eligible") == "Yes")
	pf_no = sum(1 for d in data if d.get("pf_eligible") == "No")
	pt_yes = sum(1 for d in data if d.get("pt_eligible") == "Yes")
	pt_no = sum(1 for d in data if d.get("pt_eligible") == "No")

	chart = {
		"data": {
			"labels": ["PF Yes", "PF No", "PT Yes", "PT No"],
			"datasets": [
				{"name": "Count", "values": [pf_yes, pf_no, pt_yes, pt_no]}
			]
		},
		"type":"pie"
	}
