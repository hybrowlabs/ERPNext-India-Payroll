...
def get_new_joinee_arrear(filters=None):
    ...
    if filters.get("joining_from") and filters.get("joining_to"):
        conditions["joining_date"] = [
            "between",
            [filters["joining_from"], filters["joining_to"]],
        ]
    ...
    for row in records:
        arrear_doc = frappe.get_doc("Joining Salary Arrear", row.name)
        ...
        row_data = {
            ...
            "additional_salary_date": row.additional_salary_date,
        }

        # Add earnings
        for e in arrear_doc.earning_breakup or []:
            row_data[e.salary_component] = e.amount
            all_components.add(e.salary_component)

        # Add deductions
        for d in arrear_doc.deduction_breakup or []:
...
def get_columns(components):
    base_columns = [
        ...
        {
            "label": "Present Days",
            "fieldname": "number_of_present_days",
            "fieldtype": "Float",
            "width": 120,
        },
        {
