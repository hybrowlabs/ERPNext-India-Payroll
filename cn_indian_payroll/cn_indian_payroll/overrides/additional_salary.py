...
def get_additional_salary(payroll_id, company):
    if payroll_id:
        ...
        for employee in doc1.employees:
            employee_bonus = frappe.db.get_list(
            ...
            for bonus in employee_bonus:
                ...
                if employee_id not in employee_bonus_dict:
                    employee_bonus_dict[employee_id] = {
                        "components": {},
                        "documents": [],
                    }
                ...
                if salary_component in employee_bonus_dict[employee_id]["components"]:
                    employee_bonus_dict[employee_id]["components"][
                        salary_component
                    ] += amount
                else:
                    employee_bonus_dict[employee_id]["components"][
        ...
        for employee_id, data in employee_bonus_dict.items():
            for component, total_amount in data["components"].items():
                ...
                if salary_component_doc.custom_paidout_component:
                    ...
                    frappe.msgprint(
                        f"Please set up Accrual Paid out component for {component}"
                    )
...
def additional_salary_submit(additional):
    if additional:
        additional_list = frappe.db.get_list(
            "Additional Salary",
            filters={"custom_payroll_entry": additional, "docstatus": 0},
            fields=["*"],
        )
    ...
    if self.employee:
        ...
        if accrual_list:
            ...
            if self.claimed_amount > total_amount:
                ...
                frappe.throw(
                    "Claimed amount cannot exceed total accrued amount: "
                    + str(total_amount)
                )
