...
def switch_regime(doc_id, employee, regime, company):
    ...
    if len(get_tax_regime) > 0:
        ...
        if regime == "Old Regime":
            ...
            # Uniform Allowance
            if (
                ss_assignment.custom_is_uniform_allowance
                and ss_assignment.custom_uniform_allowance_value
            ):
            ...
            update_each_doc = frappe.get_list(
                ...
                filters={
                    "employee": ss_assignment.employee,
                    "payroll_period": ss_assignment.custom_payroll_period,
                    "docstatus": ["in", [0, 1]],
                },
                fields=["*"],
            )
            ...
            if len(update_each_doc) > 0:
                for doc in update_each_doc:
                    doc1 = frappe.get_doc(
                        "Employee Tax Exemption Declaration", doc.name
                    )
