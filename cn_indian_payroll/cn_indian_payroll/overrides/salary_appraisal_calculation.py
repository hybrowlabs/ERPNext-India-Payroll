...
def appraisal_calculation(promotion_id, employee_id, company, date, effective_from):
    if promotion_id:
        ...
        if len(salary_structure_assignment) < 2:
            frappe.throw(
                "Unable to find enough salary structure assignments for comparison."
            )
        ...
        for old_earning in old_salary_slip.earnings:
            ...
            if part_of_ctc.custom_is_part_of_appraisal == 1:
                ...
                if component not in old_amounts:
                    old_amounts[component] = old_earning.amount

            if part_of_ctc.custom_is_accrual == 1:
                component = old_earning.salary_component
                if component not in old_bonus:
        ...
        all_salary_appraisal_bonus = set(old_bonus.keys()).union(set(new_bonus.keys()))
        ...
        for component in all_salary_appraisal_bonus:
        ...
        if effective_from:
            ...
            if get_all_salary_slip:
                for slip in get_all_salary_slip:
                    ...
                    get_lop_reversal = frappe.get_list(
                        "LOP Reversal",
                        filters={
                            "employee": employee_id,
                            "company": company,
                            "docstatus": 1,
                            "salary_slip": get_each_doc.name,
                        },
                        fields=["*"],
                    )
                    ...
                    actual_lop = (
                        get_each_doc.leave_without_pay + get_each_doc.absent_days
                    ) - lop_reversal
                    ...
                    for entry in result:
                        prorated_old_amount = (
                            entry["old_amount"] / get_each_doc.total_working_days
                        ) * payment_days
                        prorated_new_amount = (
                            entry["new_amount"] / get_each_doc.total_working_days
                        ) * payment_days
        ...
        if len(salary_structure_assignment) == 2:
            ...
            get_ssa_old = frappe.get_doc(
                "Salary Structure Assignment", salary_structure_assignment[1].name
            )
            ...
            for reimbursemenold in get_ssa_old.custom_employee_reimbursements:
                ...
                for reimbursement in reimbursement_array:
                    if reimbursement["component"] == reimbursemenold.reimbursements:
                        ...
                        reimbursement[
                            "old_amount"
                        ] = reimbursemenold.monthly_total_amount
                        found = True
                        break
            ...
            if effective_from:
                ...
                if get_all_salary_slip:
                    for slip in get_all_salary_slip:
                        ...
                        lop_reversal = sum(
                            [lop.number_of_days for lop in get_lop_reversal]
                        )
                            ...
                            reimbursement_final_array.append(
                                {
                                    ...
                                    "old_amount": prorated_old_amount,
                                    "new_amount": prorated_new_amount,
                                    "difference": prorated_new_amount
                                    - prorated_old_amount,
                                }
                            )
        ...
        insert_appraisal = frappe.get_doc(
            {
                ...
                "previous_salary_structure_assignment": salary_structure_assignment[
                    1
                ].name,
                "previous_effective_date": salary_structure_assignment[1].from_date,
                "revised_salary_structure_assignment": salary_structure_assignment[
                    0
                ].name,
                "revised_effective_date": salary_structure_assignment[0].from_date,
            }
        )

        for entry in result:
            insert_appraisal.append(
                "previous_revised_salary_components",
                {
                    "salary_component": entry["component"],
        ...
        for insert_arrear in final_array:
            insert_appraisal.append(
                "arrear_breakdown",
                {
                    ...
                    "payment_days": insert_arrear["payment_days"],
        ...
        for insert_bonus in final_bonus_array:
            insert_appraisal.append(
                "salary_appraisal_bonus",
                {
                    ...
                    "payment_days": insert_bonus["payment_days"],
        ...
        for insert_reimbursement in reimbursement_final_array:
            insert_appraisal.append(
                "salary_appraisal_reimbursement",
                {
                    ...
                    "payment_days": insert_reimbursement["payment_days"],
