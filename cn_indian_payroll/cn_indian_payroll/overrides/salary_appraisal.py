...
def insert_arrear_salary(self):
    ...
    if not self.salary_arrear_components:
        return

    aggregated_components = {}

    for row in self.salary_arrear_components:
        component = row.salary_component
        amount = frappe.utils.flt(row.difference)
    ...
    for component, amount in aggregated_components.items():
        ...
        salary_component_exists = frappe.db.exists(
            "Salary Component",
            {
                "custom_is_arrear": 1,
        ...
        if not salary_component_exists:
            continue

        # ✅ append ONLY on self (Document)
        self.append(
            "arrear_summary",
            {
            ...
            if benefit_accrual:
                for each_accrual_doc in benefit_accrual:
                    get_doc = frappe.get_doc(
                        "Employee Benefit Accrual", each_accrual_doc.name
                    )
...
def insert_additional_salary(self):
    ...
    if len(self.salary_arrear_components) > 0:
        ...
        result = [
            {"component": k, "amount": v} for k, v in aggregated_components.items()
        ]

        for insert in result:
            salary_component = frappe.get_list(
                "Salary Component",
                filters={
                    "custom_is_arrear": 1,
                    "custom_component": insert["component"],
                },
                fields=["*"],
            )
        ...
        for component in self.salary_appraisal_reimbursement:
            ...
            if benefit_accrual:
                for accrual in benefit_accrual:
                    get_accrued_doc = frappe.get_doc(
                        "Employee Benefit Accrual", accrual.name
                    )
