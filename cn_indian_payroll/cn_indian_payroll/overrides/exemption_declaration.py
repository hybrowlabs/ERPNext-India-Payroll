...
class CustomEmployeeTaxExemptionDeclaration(EmployeeTaxExemptionDeclaration):
    ...
    def validation_on_section10(self):
            ...
            for declaration in self.declarations:
                ...
                if component_type in [
                    ...
                    if component_type == "LTA Reimbursement":
                        ...
                        if salary_slips:
                            latest_slip = frappe.get_doc(
                                "Salary Slip", salary_slips[0].name
                            )
                        ...
                        if ss_assignment:
                            ...
                            if salary_slip_preview:
                                ...
                                payroll_period = frappe.get_doc(
                                    "Payroll Period", self.payroll_period
                                )
                                end_date = getdate(payroll_period.end_date)
                                latest_date = (
                                    getdate(latest_slip.start_date)
                                    if salary_slips
                                    else getdate(ss_assignment[0].from_date)
                                )

                                month_count = (
                                    end_date.year - latest_date.year
                                ) * 12 + (end_date.month - latest_date.month)
                                ...
                                for component in salary_slip_preview.earnings:
                                    ...
                                    if (
                                        ...
                                        future_lta_amount += (
                                            round(component.amount) * month_count
                                        )

                    total_lta = (
                        current_lta_amount + previous_lta_amount + future_lta_amount
                    )
        ...
        if unmatched:
            frappe.throw(
                _(
                    "The following Section 10 components are not part of your salary (CTC), "
        ...
        if len(self.declarations) > 0:
            ...
            if len(get_latest_history) > 0:
                ...
                each_doc.basic_as_per_salary_structure_10 = (
                    self.custom_basic_as_per_salary_structure
                )
    ...
    def calculate_hra_breakup(self):
        if self.monthly_house_rent and self.custom_check == 0:
            ...
            basic_arrears_component = get_company.custom_basic_arrear_component
            hra_component = get_company.hra_component
            hra_arrears_component = get_company.custom_hra_arrear_component
            month_count = 0
                ...
                if ss_assignment[-1].custom_payroll_period:
                    ...
                    for i in range(len(months)):
                        self.append(
                            ...
                            {
                                "month": months[i],
                                "rent_paid": round(self.monthly_house_rent),
                                "hra_received": round(
                                    self.salary_structure_hra / month_count
                                ),
            ...
            self.custom_hra_received_monthly = None
            self.custom_basic_received_monthly = None
