    ...
    def cancel_new_joinee_arrear(self):
        ...
        if joinee_arrear:
            for doc in joinee_arrear:
                doc = frappe.get_doc("Joining Salary Arrear", doc)
                doc.cancel()
                frappe.db.commit()
    ...
    def fill_employee_details(self):
        ...
        for emp in employees:
            ...
            if payroll_setting.custom_configure_attendance_cycle:
                ...
                attendance_final_end_date = datetime.date(
                    end_date.year, end_date.month, attendance_end_day
                )
                next_month_end_date = get_last_day(add_months(end_date, 1))
                next_month_attendance_end_date = datetime.date(
                    next_month_end_date.year,
                    next_month_end_date.month,
                    attendance_start_day,
                )
                diff_days = (
                    next_month_end_date - next_month_attendance_end_date
                ).days + 1

                # diff_days = (end_date - date_of_joinee).days + 1

                # print(  "\n\n\n\n\n\n\n\n\nend_date",end_date,"\n\n\n\n\n\n\n\n\n")
                # print(  "\n\n\n\n\n\n\n\n\ndiff_days",date_of_joinee,"\n\n\n\n\n\n\n\n\n")

                # print("\n\n\n\n\n\n\n\n\ndate_of_joinee",diff_days,"\n\n\n\n\n\n\n\n\n")
                        ...
                        if total_payment_days > 0:
                            ...
                            if not existing_doc:
                                arrear_doc = frappe.get_doc(
                                    {
                                        "doctype": "Joining Salary Arrear",
                                        "employee": emp.employee,
                                        "company": self.company,
                                        "number_of_present_days": total_payment_days,
                                        "posting_date": today(),
                                        "additional_salary_date": get_last_day(
        ...
        if not valid_employees:
            ...
            if self.custom_employment_type:
                error_msg += "<br>" + _("Employment Type: {0}").format(
                    frappe.bold(self.custom_employment_type)
                )
