    ...
    if employee:
        ...
        if regime == "Old Regime":
            ...
            if len(get_all_salary_slip) == 0:
                ...
                get_sub_category = frappe.get_list(
                    "Employee Tax Exemption Sub Category",
                    filters={"is_active": 1, "custom_component_type": "Provident Fund"},
                    fields=["name", "exemption_category", "max_amount"],
                    limit_page_length=1,
                )
                ...
                get_sub_category = frappe.get_list(
                    "Employee Tax Exemption Sub Category",
                    filters={"is_active": 1, "custom_component_type": "Professional Tax"},
                    fields=["name", "exemption_category", "max_amount"],
                    limit_page_length=1,
                )
                ...
                for new_earning in new_salary_slip.earnings:
                    new_earning_component = frappe.get_cached_doc(
                ...
                for old_deduction in new_salary_slip.deductions:
                    ...
                    if (
                        new_deduction_component.component_type == "Provident Fund"
                        and new_deduction_component.custom_component_sub_type == "Fixed"
                    ):
                        future_epf_amount += old_deduction.amount * old_regime_month_count

                    if (
                        new_deduction_component.component_type == "Professional Tax"
                        and new_deduction_component.custom_component_sub_type == "Fixed"
                    ):
                ...
                pt_amount_sum = (
                    (current_pt_amount + future_pt_amount) if (current_pt_amount or future_pt_amount) else 0
                )
